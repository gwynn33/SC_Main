...
from flask_login import UserMixin #importans stuff from this i can recognize the current_user!
...
try:
    ...
    class Admin_Account(db.Model,UserMixin): #this is pure abstraction ...
        ...
        def check_password(self,password):
            #checking the password if is it matching the hashed one 
            return check_password_hash(self.admin_password,password)

        def get_id(self):
            return self.admin_id
        """this function returns the admin id when we use login_user(user) in a route, that's creates a Session['__user_id'] = user.user_id"""
